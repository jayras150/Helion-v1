        ...
        <Select value={currentChatId || ""} onValueChange={handleValueChange}>
          <SelectTrigger className="h-9 min-w-0 max-w-[calc(100vw-7.5rem)] flex-1 overflow-hidden rounded-xl border-border/70 bg-background/55 px-3 shadow-sm backdrop-blur transition-all hover:border-cyan-400/60 sm:w-fit sm:flex-none sm:min-w-45 sm:max-w-72" size="sm">
            <SelectValue placeholder="Select chat">
              <div className="flex min-w-0 max-w-full items-center gap-2"><span className="flex size-5 items-center justify-center rounded-md bg-cyan-500/10 text-cyan-600 dark:text-cyan-300"><MessageSquare className="size-3.5" /></span><span className="min-w-0 truncate">{currentChat ? getChatDisplayName(currentChat) : "Select chat"}</span></div>
            </SelectValue>
          </SelectTrigger>
          <SelectContent>{chats.length > 0 ? chats.slice(0, 15).map((chat) => <SelectItem key={chat.id} value={chat.id} className="pr-16"><div className="flex min-w-0 max-w-[min(22rem,calc(100vw-3rem))] items-center gap-2"><MessageSquare className="size-4" /><span className="min-w-0 truncate">{getChatDisplayName(chat)}</span></div><button type="button" className="absolute right-7 flex size-6 items-center justify-center rounded-md text-muted-foreground hover:bg-destructive/10 hover:text-destructive" aria-label={`Delete ${getChatDisplayName(chat)}`} disabled={deletingChatId !== null} onPointerDown={(event) => { event.preventDefault(); event.stopPropagation(); }} onClick={(event) => { event.preventDefault(); event.stopPropagation(); void handleDeleteChatFromList(chat); }}>{deletingChatId === chat.id ? <Loader2 className="size-3.5 animate-spin" /> : <Trash2 className="size-3.5" />}</button></SelectItem>) : <div className="px-2 py-1.5 text-muted-foreground text-sm">No chats yet</div>}</SelectContent>
        </Select>
        {currentChat && <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="sm" className="h-9 w-9 rounded-xl border border-border/60 bg-background/45 p-0 shadow-sm" disabled={isAnyActionPending}><MoreHorizontal className="size-4" /><span className="sr-only">Chat options</span></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onClick={() => setIsDuplicateDialogOpen(true)} disabled={isAnyActionPending}><Copy className="mr-2 size-4" />Duplicate Chat</DropdownMenuItem><DropdownMenuItem onClick={() => { setSelectedVisibility(currentChat.privacy || "private"); setIsVisibilityDialogOpen(true); }} disabled={isAnyActionPending}>{getPrivacyIcon(currentChat.privacy || "private")}<span className="ml-2">Change Visibility</span></DropdownMenuItem><DropdownMenuItem onClick={() => { setRenameChatName(currentChat.name || ""); setIsRenameDialogOpen(true); }} disabled={isAnyActionPending}><Edit2 className="mr-2 size-4" />Rename Chat</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem onClick={() => setIsDeleteDialogOpen(true)} disabled={isAnyActionPending} className="text-destructive focus:text-destructive"><Trash2 className="mr-2 size-4" />Delete Chat</DropdownMenuItem></DropdownMenuContent></DropdownMenu>}
      </div>
